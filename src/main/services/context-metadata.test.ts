...
describe("active window metadata parsers", () => {
  it("parses Hyprland active window JSON", () => {
    expect(parseHyprlandActiveWindow(JSON.stringify({ class: "firefox", title: "Murmur docs - Mozilla Firefox" }))).toEqual({
      appId: "firefox",
      appName: "firefox",
  ...
  it("parses GNOME Shell Eval output", () => {
    expect(
      parseGnomeShellEvalOutput(`(true, '{"appId":"org.gnome.Terminal","appName":"Gnome-terminal","windowTitle":"Terminal"}')`)
    ).toEqual({
  ...
  it("parses X11 WM_CLASS and title output", () => {
    expect(parseX11ActiveWindow("Inbox - Mozilla Firefox", 'WM_CLASS(STRING) = "Navigator", "firefox"')).toEqual({
      appId: "firefox",
      appName: "firefox",
